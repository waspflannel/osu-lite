...
| F — Final dead-code & dependency sweep | ✅ Done | ModPreset + GlobalAction tombstones, plus broad orphan/DTO/package sweep; see below |
...
1. **Finish Phase D** (the only phase still open):
   - Notification drawer/history split: keep the transient toast/progress tray (`NotificationOverlayToastTray`) but remove the permanent drawer history (`NotificationSection`) and unread/read state. Note: the `ToggleNotifications` hotkey/binding itself has already been tombstoned in Phase F (no default binding, removed from the keybinding settings UI), so this remaining item is just the drawer/history *component* removal. Delicate — every import/maintenance/error path posts to `INotificationOverlay`, so verify progress/cancel/error toasts still appear afterward.
   - Six-section settings consolidation (cosmetic; low priority).

**Known deferred tombstones:** `ModTouchDevice`/`OsuModTouchDevice` (kept for replay/difficulty fidelity after touch input removal — see Phase E item 8).
...
### Phase E — Confirmed peripheral cuts — ✅ COMPLETED
...
- ✅ Legacy IPC/WebSocket/protocols and `.olz` — deleted `osu.Desktop/LegacyIpc/` (the TCP difficulty-calculation IPC for osu!stable), `osu.Desktop/IPC/OsuWebSocketProvider.cs` + its message types, and the underlying `osu.Game/IPC/WebSocketServer.cs`/`WebSocketChannel.cs` (no other consumer). Removed `osump://` from `WindowsAssociationManager`'s URI associations (targeted removed multiplayer behavior) and `.olz` from its file associations, plus `BeatmapExporter` (the `.olz` exporter — zero callers once removed from `BeatmapManager`) and `.olz` from `BeatmapImporter.HandledExtensions`. `.osk` was already unregistered from a prior phase; only its dead `OsuSkin` localisation string remained and is now gone too. `osu://` registration and `OsuSchemeLinkIPCChannel` are kept — they're part of the single-instance forwarding path, not a removed feature. Build-verified (`osu.Desktop` clean).
...
### Phase F — Final dead-code and dependency sweep — ✅ COMPLETED
...
- ✅ Broader cross-file orphan sweep, based on a fresh cross-file reference analysis:
  - Deleted 18 zero-consumer classes: `SupporterDisplayStrings`, `SpectatorListStrings`, `DrawableTeamFlag`/`UpdateableTeamFlag`, `Badge`, `TournamentBanner`, and the online DTOs `APIMe`, `APIMenuImage`, `APINewsPost`, `APINotification`, `APIScoreWithPosition`, `ChatSilence`, `Comment`, `CommentableMeta`, `APIChangelogBuild`/`APIChangelogEntry`/`APIChangelogUser`/`APIUpdateStream` (no changelog screen exists anywhere in the tree).
  - Pruned 8 dead `APIUser`/`UserStatistics` fields (`Achievements`, `Groups`, `DailyChallengeStatistics`, `MatchmakingStatistics`, `PlayStyles`, `RankHistory`, `MonthlyPlayCounts`, `ReplaysWatchedCounts`) together with their now-orphaned type-only classes (`APIUserAchievement`, `APIUserGroup`, `APIUserDailyChallengeStatistics`, `APIUserMatchmakingStatistics` + `APIMatchmakingPool`, `APIPlayStyle`, `APIRankHistory`, `APIUserHistoryCount`).
  - Removed `Mod.ValidForMultiplayer`/`ValidForMultiplayerAsFreeMod`/`ValidForFreestyleAsRequiredMod` from the `IMod`/`Mod` API — confirmed zero read-sites anywhere — and all ~20 override sites across `UnknownMod`, `ModTouchDevice`, `ModTimeRamp`, `ModAdaptiveSpeed`, `ModScoreV2`, `ModRateAdjust`, `ModAutoplay`, `ModSuddenDeath`, `ModPerfect`, `ModNoFail`, `ModMuted`, `ModEasy`, `ModDifficultyAdjust`, `ModClassic`, `ModHardRock`.
  - Removed `Static.LoginOverlayDisplayed` (session-only, not persisted — safe to delete outright, unlike the `GlobalAction`/`OsuSetting` enums), `EndpointConfiguration.MultiplayerUrl` (+ its two subclass assignments), and a handful of confirmed-orphaned localisation strings (`RulesetSettingsStrings.MobileLayout`/`Portrait`/`Landscape`/`LandscapeExpandedColumns`, `LeaderboardStrings.PleaseInvestInAnOsuSupporterTagToViewThisLeaderboard`, `DialogStrings.DeleteCommentBodyText`/`ConfirmExitMultiplayerMatchBodyText`, `NotificationsStrings.InvitedYouToTheMultiplayer`/`MultiplayerRoomEnded`/`MultiplayerServerShuttingDownImmediately`/`MultiplayerServerShuttingDownRemaining`).
  - Removed the `System.IO.Packaging` package reference from `osu.Desktop.csproj` (zero source consumers — leftover from a removed feature). All other package references were checked and have real consumers.
  - No empty or near-empty leftover directories were found; remaining small directories (e.g. `Overlays/FirstRunSetup`, `Performance`, `Replays`) are normal fine-grained organization, not deletion debris.
  - Build-verified after each batch (`osu.Desktop` clean).
